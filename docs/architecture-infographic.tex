\documentclass[border=10pt,varwidth=23cm]{standalone}
...
\tikzset{
  ...
  code chip/.style={
    ...
    rounded corners=1mm, fill=Fog!40, inner xsep=2mm, inner ysep=1.2mm
  },
...
\begin{pgfonlayer}{background}
  \fill[Paper,rounded corners=2.5mm] (-.5,-.5) rectangle (22.8,29.5);
\end{pgfonlayer}
...
% SECTION 2 — 技术栈与构建管线
% ══════════════════════════════════════════════════════════════════════════
\node[section kicker,anchor=west] at (0,25.6) {01 · 技术栈};
...
\node[pkg box, minimum width=28mm] (playwright) at (18.9,23.1) {Playwright};
...
% Command chips sit to the left of build boxes
\node[code chip,anchor=east] at (-0.2,23.4) {npm run build};
\node[code chip,anchor=east] at (-0.2,22.0) {npm run validate};
...
% ══════════════════════════════════════════════════════════════════════════
\node[section kicker,anchor=west] at (0,20.0) {02 · 场景家族};
...
\node[note,anchor=west,text width=5cm] at (14.9,12.0) {面要素 → 类别 + 挤压};
...
\node[pill-coral,anchor=west] at (12.8,4.9) {scripts/validate.mjs};

% Validation stages — cards with generous padding
\node[scene card, fill=white, text width=65mm, minimum height=14mm, align=left, inner xsep=3mm, inner ysep=3.5mm, anchor=west] (v1) at (12.8,4.1)
  {{\bfseries 阻断外网请求}\\{\scriptsize\color{Slate}page.route → 非 127.0.0.1 一律 abort}};

\node[scene card, fill=white, text width=65mm, minimum height=14mm, align=left, inner xsep=3mm, inner ysep=3.5mm, anchor=west] (v2) at (12.8,2.4)
  {{\bfseries 等待场景就绪}\\{\scriptsize\color{Slate}window.\_\_plotDemo.ready === true}};

\node[scene card, fill=white, text width=65mm, minimum height=14mm, align=left, inner xsep=3mm, inner ysep=3.5mm, anchor=west] (v3) at (12.8,0.7)
  {{\bfseries 断言动画帧推进}\\{\scriptsize\color{Slate}trips 场景 frame 必须递增}};

\node[scene card, fill=white, text width=65mm, minimum height=14mm, align=left, inner xsep=3mm, inner ysep=3.5mm, anchor=west] (v4) at (12.8,-1.0)
  {{\bfseries 带样式预览截图}\\{\scriptsize\color{Slate}opaque > 95\% · 彩色像素 > 3500}};

\node[scene card, fill=white, text width=65mm, minimum height=14mm, align=left, inner xsep=3mm, inner ysep=3.5mm, anchor=west] (v5) at (12.8,-2.7)
...
\node[section kicker,anchor=west] at (0,-4.6) {产物};
